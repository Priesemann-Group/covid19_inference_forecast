...
fig, axes = create_plot_scenarios(
    ...
    forecast_label=f"Increased social contacts starting Christmas",
    color="#071d58",
...
fig, axes = create_plot_scenarios(
    ...
    axes=axes,
    offset=total_cases_obs[0],
    forecast_label="No change in social contacts",
    color="#225ea8",
)
...
fig, axes = create_plot_scenarios(  # Strenger 2.nov
    ...
    axes=axes,
    offset=total_cases_obs[0],
    forecast_label=f"Strong contact reduction starting Dec 25th",
    color="tab:green",
)

# Set limit for x axes
for ax in axes:
    ax.set_xlim(datetime.datetime(2020, 9, 1), datetime.datetime(2021, 2, 1))
...
# Annotations ld_date
axes[0].axvline(
    datetime.datetime(2020, 12, 24), ls="-.", color="#99bbff", zorder=0,
)
axes[1].axvline(
    datetime.datetime(2020, 12, 24), ls="-.", color="#99bbff", zorder=0,
)
